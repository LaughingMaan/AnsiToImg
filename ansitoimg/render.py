...
def ansiToSVGRender(
    ...
    print(f"file:///{tempFileName}", size, fileName)
...
async def _doGrabWebpage(url: str, resolution: tuple[int, int], fileName: str):
    ...
    async with async_playwright() as p:
        ...
        await page.set_viewport_size({"width": resolution[0], "height": resolution[1]})  # type: ignore
        await page.goto(url)  # type: ignore
        await page.screenshot(path=fileName, omit_background=True)  # type: ignore
        await browser.close()
